...
## Build from source
All these steps below were verified to work on [Manjaro](https://manjaro.org/),  but they are likely to work on other Linux based OS as well.  Every command runs from the repository root:
...
This fetches each chapter and note over the API (sequentially, to stay gentle on the server) into `scripts/annotations-raw`.  Raw API responses are cached under `annotations-raw/.api-cache`,  so reruns don't refetch.
...
### Run the provided scripts
	...
	make -C scripts addanno MIME=../mime
...
The generated epubs land in the repository root: `ulysses-annotated.epub` (colour) and `ulysses-annotated-monochrome.epub`.
